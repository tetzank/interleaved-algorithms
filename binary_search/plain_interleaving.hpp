...
uint64_t plain_interleaving4(const int32_t *array, size_t array_size, const int32_t *keys, size_t keys_size){
	const size_t group_size=4;
	uint64_t sum=0, i=0;
	size_t size = (keys_size / group_size) * group_size;
	for(; i<size; i+=group_size){
		const int32_t *first1=array, *first2=array, *first3=array, *first4=array;
		size_t count = array_size;
		while(size_t step = count / 2){
			const int32_t *it1 = first1 + step; 
			const int32_t *it2 = first2 + step; 
			const int32_t *it3 = first3 + step; 
			const int32_t *it4 = first4 + step; 
...
uint64_t plain_interleaving8(const int32_t *array, size_t array_size, const int32_t *keys, size_t keys_size){
	...
	for(; i<size; i+=group_size){
		...
		while(size_t step = count / 2){
			...
			const int32_t *it3 = first3 + step; 
			const int32_t *it4 = first4 + step; 
			const int32_t *it5 = first5 + step; 
			const int32_t *it6 = first6 + step; 
			const int32_t *it7 = first7 + step; 
			const int32_t *it8 = first8 + step; 
			if(*it1 < keys[i+0]) first1 = it1;
			if(*it2 < keys[i+1]) first2 = it2;
			if(*it3 < keys[i+2]) first3 = it3;
			if(*it4 < keys[i+3]) first4 = it4;
...
uint64_t plain_interleaving(const int32_t *array, size_t array_size, const int32_t *keys, size_t keys_size){
	uint64_t sum=0, i=0;
	size_t size = (keys_size / group_size) * group_size;
	for(; i<size; i+=group_size){
